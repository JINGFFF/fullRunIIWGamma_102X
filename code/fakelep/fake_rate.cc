...
void count(TString type, TFile* ipt, TString str) {
...
int main(int argc, char** argv){
    ...
    TString input_file;
    if(type == "muon") input_file = "muon.txt";
    if(type == "electron") input_file = "electron.txt";
	...
	if (ww==0) tight_lepton = line;
        if (ww==1) control_lepton = line;
        if (ww==2) tight_wz = line;
        if (ww==3) control_wz = line;
	ww++;
    }
