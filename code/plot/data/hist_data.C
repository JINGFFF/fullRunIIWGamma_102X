...
void hist_data(){
        ...
        TFile * input = TFile::Open("mu.root");
        TFile* output = new TFile("hist_photon_mu.root","RECREATE");
	...
	while (fReader.Next()){
		...
		&& *drla<10 && *drj1l<10 && *drj2l<10 && *drj1a<10 && *drj2a<10
                && abs(*j1metPhi)>0.5 && abs(*j2metPhi)>0.5)) continue;
